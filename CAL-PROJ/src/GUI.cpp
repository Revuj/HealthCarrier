...
void * GUI::calcMultipleAmbulanceMultipleHealthCentersWithRestrictions(void * arg) {
	...
	Hospital<int>* typeA = new Hospital<int>(Vertex<int>(finishNodeId));
	typeA->setTreatment('A');
	hospitals.push_back(typeA);
	Hospital<int>* typeB = new Hospital<int>(Vertex<int>(finishNodeId));
	typeB->setTreatment('B');
	hospitals.push_back(typeB);
	Hospital<int>* typeC = new Hospital<int>(Vertex<int>(finishNodeId));
	typeC->setTreatment('C');
	hospitals.push_back(typeC);


	result = cg.generateMultipleAmbulancesMultipleHealthCenterWithRestrictions(hospitals, new Vertex<int>(2557));
